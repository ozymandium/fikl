...
def _get_metric_results(measure_data: pd.DataFrame, weights: pd.DataFrame, eval_order: list[str]) -> pd.DataFrame:
    ...
    eval_order : list[str]
        ...
        on other metrics, so we need to evaluate them in the correct order.
    ...
    for metric in eval_order:
        # get the factors for this metric
        factors = weights.columns[weights.loc[metric] > 0]
        # compute the results for this metric
        results[metric] = measure_data[factors].dot(weights.loc[metric, factors])
...
class Decision:
    ...
    def __init__(self, config: config_pb2, raw_path: str):
        ...
        source_data = self._get_source_data(config, raw_path, scorers)
        scorer_info = get_scorer_info_from_config(config)
        measure_data = self._get_measure_data(source_data, scorer_info)
