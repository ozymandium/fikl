...
def _json_to_binary(json_path, binary_path):
    _call(f"capnp convert json:binary {SCHEMA_PATH} Config < {json_path} > {binary_path}")


def _binary_to_json(binary_path, json_path):
    _call(f"capnp convert binary:json {SCHEMA_PATH} Config < {binary_path} > {json_path}")
...
def load(config_yaml_path: str) -> "Config":
    ...
    with open(binary_path, "rb") as f:
        struct = SCHEMA.Config.read(f)
    config = struct.to_dict()
    _check(config)
    return config
